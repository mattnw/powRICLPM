#' @title
#' Perform Power Analysis for the RI-CLPM
#'
#' @description
...
#'
#' @param target_power A numeric value between 0 and 1, denoting the desired power level for which to recommend a sample size. Use only for à priori analyses.
#' @param parameter A character string denoting the parameter for which to compute a recommended sample size. Use only for à priori analysis. See "Details" for naming defaults for parameters used in this package.
#' @param search_lower A positive integer, denoting the lower bound of the range of sample sizes for which to simulate the power. Use only for à priori analysis.
#' @param search_upper A positive integer, denoting the upper bound of the range of sample sizes for which to simulate the power. Use only for à priori analysis.
#' @param search_step A positive integer, denoting the increment in sample sizes. Use only for à priori analysis.
#' @param sample_size An integer or integer vector with elements larger than 1, indicating the sample sizes at which to evaluate power. Use only for post hoc analysis.
#' @param time_points An integer or integer vector with elements larger than 3, indicating number of time points.
...
#' @param cores A positive integer denoting the number of cores to use during the Monte Carlo simulations. Executing the analysis using multiple cores can significantly reduce computation time.
...
powRICLPM <- function(target_power = NULL,
  ...
  # Determine: À priori or post hoc power analysis
  if (!is.null(target_power)) {

    # Check à priori arguments
    check_search(search_lower, search_upper, search_step)
    target_power <- check_target(target_power)

    # Prepare and run à priori analysis
    output <- apriori(target_power = target_power,
                      parameter = parameter,
    ...
    # Check post hoc arguments
    sample_size <- check_N(sample_size)

    # Prepare and run post hoc analysis
    output <- posthoc(sample_size = sample_size,
                      time_points = time_points,
                      ICC = ICC,
                      RI_cor = RI_cor,
                      Phi = Phi,
